            ...
            <div>
                ...
                <div class="personnel">
                    <div style="display: flex">
                        <div style="width: 120px;line-height: 90px;font-size: 30px;color: #FFFFFF;">A组：</div>
                        <div v-for="item of 10" :key="item" style="margin-bottom: 10px;margin-right: 30px;">
                            <img src="../../assets/images/ren.png">
                        </div>
                    </div>
                    <div style="display: flex;margin-top: -3px;">
                        <div style="width: 120px;line-height: 90px;font-size: 30px;color: #FFFFFF;">B组：</div>
                        <div v-for="item of 10" :key="item" style="margin-bottom: 10px;margin-right: 30px;">
                            <img src="../../assets/images/ren.png">
                        </div>
                    </div>
                ...
                <div class="personnel">
                    <div  style="display: flex;">
                        <div style=" width: 120px;line-height: 90px;font-size: 30px;color: #FFFFFF;">A组：</div>
                        <div v-for="item of touxiang"  style="margin-bottom: 10px;margin-right: 30px;" v-if="item.group =='A'">
                            <div v-if="item.is_ready==1" class="tou">{{item.terminal_id}}</div>
                            <div v-if="item.is_ready==0" class="tou_">{{item.terminal_id}}</div>
                        </div>
                    </div>

                    <div style="display: flex;">
                        <div style="width: 120px;line-height: 90px;font-size: 30px;color: #FFFFFF;" v-if="touxiang">B组：</div>
                        <div v-for="item of touxiang"  style="margin-bottom: 10px;margin-right: 30px;" v-if="item.group =='B'">
                            <div v-if="item.is_ready==1" class="tou" style="background: linear-gradient(180deg, #F9902E 0%, #E24B08 100%);">{{item.terminal_id}}</div>
                            <div v-if="item.is_ready==0" class="tou_">{{item.terminal_id}}</div>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        async liebiao() {
            ...
            await this.$axios.post('term/emotion/is_ready', {id: this.$route.query.id}).then(res => {
                this.touxiangliebiao = true
            })
        },
        //游戏介绍页面
        youxi() {
         ...
         */
        play() {
            console.log('this.play',this.youxi_)
            if (this.$refs.gamePlayer) {
                console.log('this.youxi_',this.youxi_)
                this.$refs.gamePlayer.player.play(this.youxi_);
            }
        },
        ...
        pause() {
            ...
            if (this.$refs.gamePlayer) {
                if (this.num == 3 ) {
                    this.pau=setInterval(() => {
                        this.$refs.gamePlayer.player.pause(this.youxi_);
                    },1000)
                } else {
        ...
        init() {
            let url = 'ws://122.114.0.46:8296'
            // let url = 'ws://192.168.2.189:8296'
        ...
        websockMessage(e) {
            if (this.num == 3 ) {
                setTimeout(() => {
                    this.youxi_ = JSON.parse(e.data)
                    console.log('this.youxi_',this.youxi_)
                    setTimeout(() => {
                        this.youxi_.map(item => {
                            if (item.id == this.$store.state.user.terminal.userInfo.uid) {
                                this.attention = item.value
                            }
                        })
                    },300)
                },1000)
            ...
            if(this.num == 2){
                clearInterval(this.aa)
                if (JSON.parse(e.data).code == 1) {
                    this.touxiang = JSON.parse(e.data).list
                    setTimeout(() => {
                        this.jieshao()
                        clearInterval(this.yx)
                    },3000)
                ...
                console.log('this.touxiang',this.touxiang)
                return;
            }
        ...
        onReceive(data, parse) {
            ...
            //     this.status = "头戴正常"
            // }
            this.shuju = '4000'+ data + this.$store.state.webip
            if (this.num === 3) {
                this.$axios.post('term/emotion/set_status', {id: this.$route.query.id, status: 1}).then(res => {
                })
        ...
        onInstability() {
            ...
            this.pause();
            this.$axios.post('term/emotion/set_status', {id: this.$route.query.id, status: 0}).then(res => {
            })
        },
        ...
        async zhuangtai() {
             await this.$axios.post('term/emotion/set_status', {id: this.$route.query.id, status: 1}).then(res => {
...
.tou {
    width: 90px;
    height: 90px;
    background: linear-gradient(180deg, #0F71FF 0%, #0151FF 100%);
...
.tou_ {
    width: 90px;
    height: 90px;
    background: linear-gradient(180deg, #CDCDCD 0%, #999999 100%);
    border: 2px solid #FFFFFF;
    border-radius: 50%;
    font-size: 24px;
    font-weight: 500;
